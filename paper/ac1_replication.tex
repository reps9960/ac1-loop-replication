...
reimplementation of the loop and subject four of its claims to those controls.
...
produces strong hysteresis (path-dependent memory) but no sharp bistable jump,
and this hysteresis \emph{survives a decay-free filter} ($\rho=1.0$), showing it
to be an emergent property of the loop rather than an artefact of the estimator.
...
make any above-chance system look impressive; and an estimator with decay will
look path-dependent whether or not the surrounding loop is. The originating
...
loop. We make \textbf{no} claim about the Ψ-threshold, consciousness, or the
...
\paragraph{The decay confound, and its resolution.} The tracker's decay
($\rho=0.97<1$) builds intrinsic inertia into the posterior, so some stickiness
is mechanical rather than emergent. The decisive control is a decay-free rerun.
With $\rho=1.0$ the loop area held at $0.115$ (vs.\ $0.126$)---essentially
unchanged (Table~\ref{tab:hyst}). Had the stickiness been filter inertia,
removing decay would have collapsed the loop area toward zero; it did not.
The path-dependence therefore \textbf{survives a memoryless filter} and is an
emergent property of the closed loop, not an artefact of the estimator. The
transition remains smooth under both settings.
...
Across four controls the picture is consistent: each tested claim reproduces in a
\emph{true-but-softer} form. The self-state is causally load-bearing on
...
contributes persistence rather than accuracy. And the closed loop has genuine,
emergent path-dependence, confirmed to survive a memoryless filter---though as
smooth hysteresis, not a sharp transition.
...
single-shot baseline. These are the natural next experiments.

\section*{Reproducibility}
A clean-room implementation of the loop and all four experiments, with the raw
