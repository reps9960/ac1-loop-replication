...
\author{Grant\thanks{Independent researcher (Franks.Games). Correspondence via the
project repository.}}
...
strongest.
\end{itemize}
In a companion two-stage variant on a non-reasoning base (so the monologue is the
only deliberation), \textsc{true}/\textsc{scrambled}/\textsc{null} produced reply
lengths of $\sim$131/47/79 words---opposite-signed departures from null in the
state-consistent direction ($t\approx5.2$ for \textsc{true} vs.\ \textsc{scrambled}).
Chained with the \textsc{numbers\_only}${\approx}$\textsc{null} result, this
